# -*- coding: utf-8 -*-
r"""
V20 - SURFACE VOLATILITY PROCESSOR - PERMA SINGLE INSTANCE
===========================================================

🔥 NUEVO EN V20 - CORRECCIONES CRÍTICAS:
✅ FIX CRÍTICO #1: Cálculo de percentil empírico corregido
   - Ahora usa scipy.stats.percentileofscore (método 'mean')
   - Elimina sesgo sistemático de ~12.5% en extremos (0-10%, 90-100%)
   - Clasificaciones ULTRA_BARATA y ULTRA_CARA ahora precisas

✅ FIX CRÍTICO #2: Scores ATM/OTM unificados y comparables
   - Pesos consistentes en todos los buckets (60% IV, 35% SKEW, 5% VRP)
   - Para ATM: SKEW_pct rellenado con 0.5 (neutral) si NaN
   - Ahora los scores son comparables cross-bucket (ATM vs OTM)
   - Rankings y estrategias multi-strike ahora válidos
...
✅ FIX 1: Eliminación de filas fantasma (completamente vacías)
✅ FIX 2: Reindex solo desde primer dato real del bucket (no antes)
✅ FIX 3: Manejo robusto de NaN en sorted() y estadísticas
...
CLOSE_MS = 15 * 60 * 60 * 1000 + 30 * 60 * 1000  # 15:30
...
def _process_single_file(f: Path) -> Tuple[List[dict], Dict]:
    ...
    try:
        ...
        scale = _delta_scale(s10["delta_BS"])
        s10.loc[:, "delta_norm"] = s10["delta_BS"] / scale
        s10.loc[:, "delta_abs"] = s10["delta_norm"].abs()
        s10.loc[:, "dte_days"] = compute_dte_days(s10["date"], s10["expiration"])
        
        spx_col = _spx_col_available(s10)
        spot10 = float(s10[spx_col].median()) if spx_col and spx_col in s10.columns else np.nan
        
        s10.loc[:, "spread_abs"] = s10["ask"] - s10["bid"]
        with np.errstate(divide='ignore', invalid='ignore'):
            s10.loc[:, "spread_pct"] = (s10["ask"] - s10["bid"]) / s10["mid"]
            s10.loc[:, "ask_bid_ratio"] = s10["ask"] / s10["bid"]
        ...
        cond10 = (
            ...
            (s10["IV"].notna()) &
            (s10["bid"] <= s10["ask"]) &
            (s10["spread_abs"] <= ABS_SPREAD_MAX) &
            (s10["spread_pct"] <= PCT_SPREAD_MAX) &
            (s10["ask_bid_ratio"] <= MAX_ASK_BID_RATIO)
        )
        ...
        for exp, bloc in s10.groupby("expiration"):
            if bloc.empty:
                continue
        ...
        # IV_ATM_30D
        s10.loc[:, "day_norm"] = pd.to_datetime(s10["date"]).dt.normalize()
        day = s10["day_norm"].iloc[0]
