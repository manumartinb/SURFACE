# -*- coding: utf-8 -*-
r"""
V19_rev2 - SURFACE VOLATILITY PROCESSOR - PERMA SINGLE INSTANCE
================================================================
...
NUEVO EN V19_rev1 (PERMA SINGLE INSTANCE):
...
def calculate_bucket_percentiles(
    ...
    for (wing, dcode, tcode), g in iterator:
        ...
        for W in WINDOWS:
            ...
            if is_atmish:
                denom = (w_iv + w_vrp) if (w_iv + w_vrp) > 0 else 1.0
                wiv = w_iv / denom
                wvr = w_vrp / denom
                gg[f"SCORE_SIMPLE_{W}"] = (
                    wiv * gg[f"IV_pct_{W}"] + wvr * gg[f"VRP_pct_{W}"]
                )
            else:
                gg[f"SCORE_SIMPLE_{W}"] = (
                    w_iv * gg[f"IV_pct_{W}"] +
                    w_sk * gg[f"SKEW_pct_{W}"] +
